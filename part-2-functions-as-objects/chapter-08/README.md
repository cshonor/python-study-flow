...
**一键总览（学习路线 + 命令 + 避坑，与各 `01`～`07` 笔记互为索引）**：[00 全套类型提示总览](<00-全套类型提示深度展开（Fluent Python 第8章学习路线）.md>)（推荐先扫一遍目录与 § 对应表，再按序读各节正文）。
...
| 文件 | 说明 |
|---|---|
| `00-全套类型提示深度展开（Fluent Python 第8章学习路线）.md` | **总览**：01～07 路线表、仓库根目录 `mypy`/`python` 命令、量化迁移阶段、避坑；与各节 `NN-…md` 配套 |
