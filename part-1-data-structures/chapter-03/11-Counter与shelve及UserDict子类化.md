...
### 一）`Counter`：带「计数代数」的字典

**一句话**：**在频次表上还能做加减、交并**的超级字典子类。

- 为**可哈希元素**计数；**`update`** 是**累加**，不是整条覆盖。  
...
1. **`Counter`**：**数数**还能算——**`most_common`、加减交并一把梭**。  
2. **`shelve`**：**落盘**像 `dict`——**字符串键、`pickle` 值、别信陌生文件**。  
3. **`UserDict`**：**自定义**别硬啃 `dict`——**`self.data` 在手，扩展少翻车**。
