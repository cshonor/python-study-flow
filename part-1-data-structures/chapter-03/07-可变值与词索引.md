...
## 零、知识卡片版（可整段复制到 PPT / Canva / 长图）

> 下面为**极简排版文案**；技术细节与脚本见 **§一～§九**。

---

### 字典里放可变值（list）：缺了就建 + append

**核心需求**  
第一次见词 → 挂新 `[]`；再见到 → 同一列表 `append`。  
**难点**：缺键时怎么初始化才好看、又少踩坑。
...
✅ 好懂、不 `KeyError`  
❌ 三步；且 **`[]` 每次调用都会先被求值**，键已存在时多造一个随即丢弃的空列表（见 **§二**）

**2）`setdefault`（1 行，书里常见）**
...
✅ 一行、不必手写写回  
⚠️ **`[]` 仍是每次调用都先求值**；键已存在时同样会多分配丢弃的 `[]`（见 **§四**）

**3）`defaultdict(list)`（高频场景最省事）**
...
1. **别让所有键共用一个 list**  
   ❌ `shared = []` 传给 `setdefault` / `fromkeys`  
   ✅ 每次 `[]` 或 **`defaultdict(list)`**

2. **只查不写 → `get`；要追加/原地改 → `setdefault` / `defaultdict`**

3. **选型一句**  
   偶尔补 → `setdefault`；**高频分组/索引 → `defaultdict(list)`**

---

### 一句话结论

**词 → 位置列表** 这类模式：**`defaultdict(list)` + `index[word].append(...)`** 往往最干净；普通 `dict` 且出现不频繁时 **`setdefault`** 也很够用。
