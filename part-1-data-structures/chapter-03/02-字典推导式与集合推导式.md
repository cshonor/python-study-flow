...
assert filtered[1] == "UNITED STATES" and filtered[7] == "RUSSIA"  # 成立 → 区号作键、国名大写；仅 code<70
```

含义拆解：

1. **`sorted(country_dial.items())`**：对 `items()` 给出的 **`(country, code)`** 序列排序；默认按元组首元素即**国家名**排。  
2. **`items()`**：迭代字典时逐项得到 **`(键, 值)`** 元组（这里是 `(国家名, 区号)`）；返回的是**视图**，不是复制整张表。  
3. **`code: country.upper()`**：以区号为键、国家名为大写字符串为值（与 `country_dial` 中「国名→区号」再次对调并格式化）。  
4. **`if code < 70`**：只保留区号小于 70 的项。
...
## 七、自测练习（参考答案见 `02_dict_comprehension_demo.py` 第 5 段）
...
见 `02_dict_comprehension_demo.py`（区号示例、进阶过滤、重复键、`set` 推导式、自测答案）。
